...
test.describe('Demo video — categories', () => {
  test('Category: Ownership', async ({ page }) => {
    ...
    await beat(page, 4850); // cue 5 (part 1 of 2)

    await humanClickLocator(page, settingsControl(settingsPanel, 'Show Sprints:'));
    await beat(page, 4850); // cue 5 (part 2 — kept at beat index only, first part above anchors the cue's real timing)
