...
test.describe('Demo video — categories', () => {
  test('Category: Expertise', async ({ page }) => {
    ...
    // ─── Code Expertise ─────────────────────────────────────────────────────────────────────
    {
      ...
      await beat(page, 48800); // cues 1+2+3+4 — holds through 4 narration cues describing this one segment (size, bands, arc, dotted count)
      ...
      if (branchOptionCount > 1) {
        ...
        await beat(page, 8630); // cue 5 — narrates the Branch selector itself
        // Show the ring chart re-rendered for the newly selected branch, hovering a real segment rather than the center.
        await hoverRankedElement(page, item, 'path[id$="_commitsPath"]', HOVER_STEPS);
        await beat(page, 5370); // cue 6 — "the tempting read: resident expert"
      }

      await closeSubWindow(page, settingsPanel);
      await beat(page, 25200); // cue 7 — long "be careful with expert" correction, held on the closed panel
    }
    ...
    // No settings component for this plugin.
    {
      ...
      await beat(page, 10720); // cue 8 — intro: "Knowledge Radar plots expertise differently..."
      ...
      for (let i = 0; i < packagePath.length; i++) {
        ...
        await beat(page, 1200); // cue 9 (first iteration only) — "clicking drills deeper..." spread across this whole loop
      ...
      if (await multiAuthorComponent.count()) {
        ...
        await beat(page, 51520); // cues 10+11
      }
    }
