        ...
        {exportType === ExportType.all && (
          <div className={dataExportStyles.chartContainer}>
            <div className={dataExportStyles.mg1}>
                ...
                {Object.keys(state.collections).map((c) => {
                  return (
                    <div key={c}>
                      ...
                      {state.collections[c as keyof typeof state.collections].length > 10000
                        ? '(Too many Entries! Preview may crash Binocular.)'
                        : ''}
                      <img
                ...
                {Object.keys(state.relations).map((r) => {
                  return (
                    <div key={r}>
                      ...
                      {state.relations[r as keyof typeof state.relations].length > 10000
                        ? '(Too many Entries! Preview may crash Binocular.)'
                        : ''}
                      <img
                        className={dataExportStyles.icon}
              ...
              <hr />
              <button
                className={'button ' + dataExportStyles.button}
                onClick={() => {
                  DataPluginStorage.getDataPlugin(selectedDataPlugin!).then((dataPlugin) => {
                    // eslint-disable-next-line
                    dataPlugin?.export ? dataPlugin.export(selectedDataPlugin?.metadata) : {};
                  });
                }}>
