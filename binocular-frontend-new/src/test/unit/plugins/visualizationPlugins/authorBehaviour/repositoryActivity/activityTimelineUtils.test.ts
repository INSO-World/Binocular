...
describe('convertToActivityTimelineFormat', () => {
  it('U52.1 empty array returns empty chartData', () => {
    const { chartData } = convertToActivityTimelineFormat([]);
    expect(chartData).toHaveLength(0);
  });

  it('U52.2 single activity returns one entry with value 1', () => {
    const { chartData } = convertToActivityTimelineFormat([makeCommit('2024-01-15')]);
    expect(chartData).toHaveLength(1);
    expect(chartData[0].value).toBe(1);
  });

  it('U52.3 two activities on same day collapse to one entry with value 2', () => {
  ...
  it('U52.4 activities on different days produce separate entries', () => {
    const { chartData } = convertToActivityTimelineFormat([makeCommit('2024-01-15'), makeCommit('2024-01-16')]);
    expect(chartData).toHaveLength(2);
  });

  it('U52.5 branch activity without latestCommit (null date) is skipped', () => {
  ...
  it('U52.6 output is sorted ascending by date', () => {
