 ...
 */
internal class ModuleControllerWebTest : BaseIntegrationTest() {
    @Nested
    inner class BasicFunctionality : GraphQlControllerTest() {
        ...
        """,
            ...
            TestDataProvider.testModules.forEachIndexed { index, expectedModule ->
                ...
                assertAll(
                    {
                        assertEquals(
                            expectedModule.id,
                            actualModule.get("id").asText(),
                            "Module ID mismatch: expected ${expectedModule.id}, got ${actualModule.get("id").asText()}",
                        )
                    },
                    {
                        assertEquals(
                            expectedModule.path,
                            actualModule.get("path").asText(),
                            "Module path mismatch: expected ${expectedModule.path}, got ${actualModule.get("path").asText()}",
                        )
                    },
                )
            }
        ...
        fun `should return module by id`() {
            val expectedModule = TestDataProvider.testModules.first { it.id == "1" }
            ...
            assertAll(
                {
                    assertEquals(
                        ...
                        "Module ID mismatch: expected ${expectedModule.id}, got ${result.get("id").asText()}",
                    )
                },
                {
                    assertEquals(
                        ...
                        "Module path mismatch: expected ${expectedModule.path}, got ${result.get("path").asText()}",
                    )
                },
            )
        }
    ...
    inner class Pagination : GraphQlControllerTest() {
        ...
        """,
            ...
            val expectedModule = TestDataProvider.testModules.first()
        ...
        """,
            ...
            val expectedModule = TestDataProvider.testModules[1]
            val actualModule = modulesData.get(0)
