 ...
 */
internal class NoteControllerWebTest : BaseIntegrationTest() {
    @Nested
    inner class BasicFunctionality : GraphQlControllerTest() {
                ...
                assertAll(
                    {
                        assertEquals(
                            expectedNote.id,
                            actualNote.get("id").asText(),
                            "Note ID mismatch: expected ${expectedNote.id}, got ${actualNote.get("id").asText()}",
                        )
                    },
                    {
                        assertEquals(
                            expectedNote.body,
                            actualNote.get("body").asText(),
                            "Note body mismatch: expected ${expectedNote.body}, got ${actualNote.get("body").asText()}",
                        )
                    },
                    {
                        assertEquals(
                    ...
                    },
                    {
                        assertEquals(
                            expectedNote.system,
                            actualNote.get("system").asBoolean(),
                            "Note system mismatch: expected ${expectedNote.system}, got ${actualNote.get("system").asBoolean()}",
                        )
                    },
                    {
                        assertEquals(
                            expectedNote.resolvable,
                            actualNote.get("resolvable").asBoolean(),
                            "Note resolvable mismatch: expected ${expectedNote.resolvable}, got ${actualNote.get(
                                "resolvable",
                            ).asBoolean()}",
                        )
                    },
                    {
                        assertEquals(
                            expectedNote.confidential,
                            actualNote.get("confidential").asBoolean(),
                            "Note confidential mismatch: expected ${expectedNote.confidential}, got ${actualNote.get(
                                "confidential",
                            ).asBoolean()}",
                        )
                    },
                    {
                        assertEquals(
                            expectedNote.internal,
                            actualNote.get("internal").asBoolean(),
                            "Note internal mismatch: expected ${expectedNote.internal}, got ${actualNote.get("internal").asBoolean()}",
                        )
                    },
                    {
                        assertEquals(
                            expectedNote.imported,
                            actualNote.get("imported").asBoolean(),
                            "Note imported mismatch: expected ${expectedNote.imported}, got ${actualNote.get("imported").asBoolean()}",
                        )
                    },
                    {
                        assertEquals(
                            expectedNote.importedFrom,
                            actualNote.get("importedFrom").asText(),
                            "Note importedFrom mismatch: expected ${expectedNote.importedFrom}, got ${actualNote.get(
                                "importedFrom",
                            ).asText()}",
                        )
                    },
        ...
        fun `should return note by id`() {
            val expectedNote = TestDataProvider.testNotes.first { it.id == "1" }
            ...
            assertAll(
                {
                    assertEquals(
                        ...
                        "Note ID mismatch: expected ${expectedNote.id}, got ${result.get("id").asText()}",
                    )
                },
                {
                    assertEquals(
                        ...
                        "Note body mismatch: expected ${expectedNote.body}, got ${result.get("body").asText()}",
                    )
                },
                {
                    assertEquals(
                ...
                },
                {
                    assertEquals(
                        expectedNote.system,
                        result.get("system").asBoolean(),
                        "Note system mismatch: expected ${expectedNote.system}, got ${result.get("system").asBoolean()}",
                    )
                },
                {
                    assertEquals(
                        expectedNote.resolvable,
                        result.get("resolvable").asBoolean(),
                        "Note resolvable mismatch: expected ${expectedNote.resolvable}, got ${result.get("resolvable").asBoolean()}",
                    )
                },
                {
                    assertEquals(
                        expectedNote.confidential,
                        result.get("confidential").asBoolean(),
                        "Note confidential mismatch: expected ${expectedNote.confidential}, got ${result.get("confidential").asBoolean()}",
                    )
                },
                {
                    assertEquals(
                        expectedNote.internal,
                        result.get("internal").asBoolean(),
                        "Note internal mismatch: expected ${expectedNote.internal}, got ${result.get("internal").asBoolean()}",
                    )
                },
                {
                    assertEquals(
                        expectedNote.imported,
                        result.get("imported").asBoolean(),
                        "Note imported mismatch: expected ${expectedNote.imported}, got ${result.get("imported").asBoolean()}",
                    )
                },
                {
                    assertEquals(
                        expectedNote.importedFrom,
                        result.get("importedFrom").asText(),
                        "Note importedFrom mismatch: expected ${expectedNote.importedFrom}, got ${result.get("importedFrom").asText()}",
                    )
                },
            )
        }
            ...
            assertAll(
                ...
                },
                {
                    assertEquals(
                        expectedNote.importedFrom,
                        actualNote.get("importedFrom").asText(),
                        "Note importedFrom mismatch: expected ${expectedNote.importedFrom}, got ${actualNote.get("importedFrom").asText()}",
                    )
                },
            )
        }
