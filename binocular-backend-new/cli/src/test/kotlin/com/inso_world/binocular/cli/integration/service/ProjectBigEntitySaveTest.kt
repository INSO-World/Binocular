...
internal class ProjectBigEntitySaveTest @Autowired constructor(
    ...
    fun `save project with full graph including repository and issues`() {
        ...
        val issue2 = Issue().apply {
            title = "Issue 2"
        }

        val mr1 = MergeRequest(
            title = "Merge Request 1",
            description = "Implementing feature X",
            createdAt = LocalDateTime.now().toString(),
            state = "opened"
        ).apply {
            notes = listOf(Note(body = "MR Note", createdAt = LocalDateTime.now().toString(), updatedAt = LocalDateTime.now().toString(), importedFrom = "test"))
            milestones = listOf(milestone1)
