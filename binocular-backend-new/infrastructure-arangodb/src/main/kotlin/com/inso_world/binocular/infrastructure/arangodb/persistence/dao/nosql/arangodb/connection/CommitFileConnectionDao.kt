...
constructor(
    ...
    override fun findFileComplexityForAllFiles(): List<FileComplexityMinorContributors> {
        val rows = repository.findFileComplexityForAllFiles()
        return rows.mapNotNull { row ->
            val path = row["filePath"]?.toString() ?: return@mapNotNull null
            FileComplexityMinorContributors(
                filePath = path,
                loc = (row["loc"] as? Number)?.toInt() ?: 0,
                minorContributors = (row["minorContributors"] as? Number)?.toInt() ?: 0,
                nfix = (row["nfix"] as? Number)?.toInt() ?: 0,
            )
        }
    }
