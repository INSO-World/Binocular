...
class V006_AddBranchName : Migration {
    companion object {
        private val logger by logger()
    }

    override val version = 1771592319
    override val description = "Add name property to branch"
    ...
    private fun backfillBranchRepositoryRef(db: ArangoDatabase) {
        val result =
            db
                .query(
                    """
                    FOR c IN branches
                    LET branch_name = c.branch
                    UPDATE c WITH { 
                        name: branch_name,
                        fullName: branch_name,
                        category: 'REMOTE_BRANCH'
                    } IN branches
                    RETURN 1
                    """.trimIndent(),
                    Int::class.java,
        ...
        if (updatedCount == 0) {
            logger.info("No branches need a name set")
        } else {
            logger.info("Updated {} branch(es) with name property", updatedCount)
        }
    }
}
