...
internal class ProjectAssemblerTest : BaseAssemblerTest() {
    private fun createCommit(
        ...
        message: String? = null,
        committer: Developer? = null,
    ): Commit {
        val resolvedCommitter =
            committer ?: Developer(
    ...
    inner class ToEntity {
        ...
        fun `Project with Repository, check complete structure`() {
            ...
            val repository =
                TestData.Domain.testRepository(
                    localPath = "/test/repo",
                    project = project
                )
        ...
        fun `Project with Repository containing commits and branches, check full graph`() {
            ...
            assertAll(
                ...
                { assertThat(entity.repo?.developers).hasSize(1) }
            )
        }
        ...
        fun `Project with Repository already assembled, Repository uses cached entity`() {
            ...
            // Assemble project first (creates ProjectEntity in context)
            val projectEntity = projectAssembler.toEntity(project)

            clearMocks(ctx, projectMapper, repositoryAssembler, recordedCalls = true)

            // Assemble repository (should find cached project, reuse it)
            val repoEntity = repositoryAssembler.toEntity(repository)

            assertThat(repoEntity.project).isSameAs(projectEntity)
        ...
        fun `Project with Issues, maps author and assignees correctly`() {
            ...
            val issue =
                TestData.Domain.testIssue(project = project, author = author).apply {
            ...
            // Note: author is wired to issueEntity.author
            // assignees should be mapped to issueEntity.accounts via accountMapper
            assertAll(
    ...
    inner class ToDomain {
        ...
        fun `ProjectEntity with full Repository graph, check complete domain`() {
            ...
            val commit1Entity =
                CommitEntity(
                    sha = "a".repeat(40),
                    authorDateTime = LocalDateTime.of(2020, 1, 1, 0, 0),
                    commitDateTime = LocalDateTime.of(2020, 1, 1, 0, 0),
                    message = null,
                    repository = repositoryEntity,
                    author =
                        TestData.Entity.testDeveloperEntity(
                            name = "Author-aaaaaa",
                            email = "author-aaaaaa@example.com",
                            repository = repositoryEntity
                        ),
                    committer =
                        TestData.Entity.testDeveloperEntity(
                            name = "Committer-aaaaaa",
            ...
            val commit2Entity =
                CommitEntity(
                    sha = "b".repeat(40),
                    authorDateTime = LocalDateTime.of(2020, 1, 2, 0, 0),
                    commitDateTime = LocalDateTime.of(2020, 1, 2, 0, 0),
                    message = null,
                    repository = repositoryEntity,
                    author =
                        TestData.Entity.testDeveloperEntity(
                            name = "Author-bbbbbb",
                            email = "author-bbbbbb@example.com",
                            repository = repositoryEntity
                        ),
                    committer =
                        TestData.Entity.testDeveloperEntity(
                            name = "Committer-bbbbbb",
        ...
        fun `ProjectEntity with Issues maps author and assignees correctly`() {
            ...
            val assignee =
                TestData.Entity.testAccountEntity(
                    login = "assignee",
                    gid = "MJDKJD234OD",
            ...
            val issue =
                TestData.Entity.testIssueEntity(project = projectEntity).apply {
                    this.author = author
                    this.accounts.add(assignee)
                }
            ...
            assertAll(
                { assertThat(issueDomain.author?.login).isEqualTo("author") },
                {
                    assertThat(issueDomain.accounts.map { it.login })
                        .containsExactlyInAnyOrder("author", "testlogin", "assignee")
                }
            )
        }
    }
