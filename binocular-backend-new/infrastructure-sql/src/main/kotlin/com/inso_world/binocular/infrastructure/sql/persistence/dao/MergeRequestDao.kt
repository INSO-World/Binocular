...
internal class MergeRequestDao {
    ...
    fun findById(id: String): MergeRequest? =
        try {
            em
                .createNativeQuery("select id, title, state from merge_requests where id = ?1")
                .setParameter(1, id)
                .resultList
                .firstOrNull()
                ?.let { row ->
    ...
    fun findAll(): List<MergeRequest> =
        em
            .createNativeQuery("select id, title, state from merge_requests order by id")
            .resultList
            .map { row ->
                val arr = row as Array<*>
                MergeRequest(
    ...
    fun count(): Long = (em.createNativeQuery("select count(*) from merge_requests").singleResult as Number).toLong()

    fun findAll(pageable: Pageable): List<MergeRequest> =
        em
            .createNativeQuery("select id, title, state from merge_requests order by id limit ?1 offset ?2")
    ...
    fun create(mr: MergeRequest): MergeRequest {
        em
            .createNativeQuery("insert into merge_requests (id, title, state, created_at, updated_at) values (?1, ?2, ?3, ?4, ?5)")
    ...
    fun update(mr: MergeRequest): MergeRequest {
        em
            .createNativeQuery("update merge_requests set title = ?1, state = ?2 where id = ?3")
    ...
    fun deleteById(id: String) {
        em
            .createNativeQuery("delete from merge_requests where id = ?1")
            .setParameter(1, id)
            .executeUpdate()
    }
